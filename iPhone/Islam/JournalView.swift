...
struct JournalView: View {
    ...
    /// In Al-Islam these are the notes left on bookmarked ayahs and hadiths. This app has
    /// neither store, so there are none to gather and the "Margin notes" filter hides itself.
...
struct JournalAttachSheet: View {
    ...
    var body: some View {
        SheetNavigationContainer {
            List {
                Group {
                    // (Al-Islam also attaches an ayah by reference and a bookmarked hadith here;
                    // this app carries neither the Quran text nor a hadith library.)
