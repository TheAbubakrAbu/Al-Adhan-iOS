    ...
    private var versesSection: some View {
        ...
        Section(header: SectionPillHeader(title: "IN THE QURAN", count: verses.count)) {
            if verses.isEmpty {
                ...
                // Al-Islam prints each verse in full and opens it in the reader. This app ships no
                // Quran text, so the names are cited by reference alone.
                ForEach(verses) { verse in
                    VStack(alignment: .leading, spacing: 2) {
                        Text(verbatim: "Quran \(verse.surah):\(verse.ayah)")
                            .font(.subheadline.weight(.semibold))
                            .foregroundColor(accent)
                        if verse.tokens.isEmpty {
                            Text("Named in another form here")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    }
