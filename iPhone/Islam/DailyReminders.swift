...
final class DailyReminderStore: ObservableObject {
    ...
    /// and translation, the Fortress entry's Arabic for a dua the Fortress carries. Runs where the
    /// parse runs, off the main thread, once; after it a card's `arabic` and `english` are plain
    /// strings, so nothing downstream (the card, a saved reflection) knows the difference.
    /// A reference that cannot be read leaves the card's text empty rather than stale.
    ///
    /// (Al-Islam reads the narration from the bundled .hpk shelf. This app ships no shelf, so it
    /// reads `HadithQuotes.json.deflate` through `HadithQuoteSource` - which is why that pack carries
    /// the reminder links as well as the articles', unlike the Watch's copy upstream.)
    private static func resolveReferences(_ entries: [DailyReminderEntry]) -> [DailyReminderEntry] {
        ...
        return entries.map { entry in
            ...
            if let link = entry.hadith, entry.arabicWords != nil || entry.englishWords != nil {
                if let reference = HadithQuoteReference("\(link.slug):\(link.citation)"),
                   let text = HadithQuoteSource.read(reference) {
