...
enum MiracleBlock {
    ...
    /// A cited verse or short range. Al-Islam prints the text from `QuranData`; here it is cited by
    /// reference (this app ships no Quran text).
    ...
    func plainText() -> String {
        ...
        for block in blocks {
            ...
            case .ayah(let surah, let first, let endAyah):
                // No Quran text in this app: the citation travels as a reference.
                let last = min(endAyah ?? first, first + 4)
                parts.append(last > first ? "Quran \(surah):\(first)-\(last)" : "Quran \(surah):\(first)")
            ...
            ToolbarItem(placement: .navigationBarTrailing) {
                Menu {
                    Button {
                        ...
                        UIPasteboard.general.string = article.plainText()
                    ...
                    Button {
                        Settings.shared.hapticFeedback()
                        presentSystemShareSheet(items: [article.plainText()])
                    } label: {
...
struct MiracleBlockView: View {
    ...
    var body: some View {
        ...
        case .closer(let text, let links):
            ...
            .padding(.top, 4)
        case .ayah(let surah, let first, let endAyah):
            // Al-Islam prints the cited verses in full here. This app ships no Quran text, so the
            // citation is the reference itself; the article's own prose carries the argument.
            let last = min(endAyah ?? first, first + 4)
            Label(last > first ? "Quran \(surah):\(first)-\(last)" : "Quran \(surah):\(first)",
                  systemImage: "character.book.closed.ar")
                .font(.subheadline.weight(.semibold))
