...
private enum ActivityGoal {
    /// Today's targets the rings fill toward: a modest reading default, one surah listened, a
    /// hundred counts of dhikr. (Al-Islam reads the first from its Quran reading plan; this app
    /// ships no planner, so the default stands.)
    @MainActor
    static func read() -> Int { 10 }
    static let listen = 1
