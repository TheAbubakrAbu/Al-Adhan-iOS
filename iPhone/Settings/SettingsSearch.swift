...
// this file gives each settings page (Prayer, Notifications, Appearance, Credits) a search bar of its
// own over that page's slice of the same index, with results that push the exact sub-screen
// (Traveling Mode, Sky Colors, ...) instead of landing on the page's root. The sub-screens are named
// by the page enums below, which the root search's deep links use too. Al-Islam's Quran and hadith
// pages have no counterpart in this app, so their scopes and entry lists are not here.
...
/// (Al-Islam also lists Sunnah Reminders here; those schedule recitations in the Quran reader this
/// app does not ship.)
...
/// iOS Settings app's grammar). The content areas are FIXED whatever accent the user picked
/// (Abu, 2026-09-16): prayer yellow here, as in Al-Islam.
enum SettingsTint {
...
enum SettingsSearchScope: Hashable {
    case notifications, prayer, appearance, credits
...
extension SettingsSearchEntry.Destination {
    ...
    var isPageRoot: Bool {
        switch self {
        case .notifications, .prayerSettings, .appearance, .credits: return true
