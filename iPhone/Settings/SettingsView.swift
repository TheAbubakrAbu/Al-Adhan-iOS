...
struct SettingsSearchEntry: Identifiable {
    ...
    enum Destination {
        ...
        /// This app has no Quran or hadith settings pages.
...
struct SettingsView: View {
    ...
    private static var debugSettingsPage: SettingsSearchEntry.Destination? {
        // Only the pages this app has (no Quran or hadith settings here).
    ...
    // The index is composed here from the per-screen entry lists (see `SettingsSearchEntry`); the
    // ranking is `SettingsSearchEntry.rank` and the destination mapping is below. This app has no
    // Quran or hadith settings pages, so their entry lists are not in the sum.
    ...
    /// Each area wears its own fixed colour (`SettingsTint`: prayer yellow here, as in Al-Islam),
    ...
    private var resetSection: some View {
        ...
        Section(header: Text("RESET")) {
            ...
            .confirmationDialog(
                ...
                Text("Reset restores every setting (appearance and prayer options) to its default and keeps your bookmarks, favorites, khatm progress, and saved location.\n\nErase removes those too.")
            }
